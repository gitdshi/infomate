...
export default {
	...
	data() {
		return {
			state: loadState('infomate', 'notes-initial-state'),
		}
	},
	...
	methods: {
		onEditNote(noteId, content) {
			...
			const url = generateOcsUrl('apps/infomate/api/v1/notes/{noteId}', { noteId })
			axios.put(url, options).then(response => {
				...
				showError(t('infomate', 'Error saving note content'))
				console.error(error)
			})
		},
		onCreateNote(name) {
			...
			const url = generateOcsUrl('apps/infomate/api/v1/notes')
			axios.post(url, options).then(response => {
				...
				showError(t('infomate', 'Error creating note'))
				console.error(error)
			})
		},
		onDeleteNote(noteId) {
			...
			showUndo(
				t('infomate', '{name} deleted', { name: this.notesById[noteId].name }),
				() => {
					deletionTimer.pause()
		...
		deleteNote(noteId) {
			...
			axios.delete(url).then(response => {
				...
				showError(t('infomate', 'Error deleting note'))
				console.error(error)
			})
		},
		onClickNote(noteId) {
			...
			const url = generateUrl('apps/infomate/config')
			axios.put(url, options).then(response => {
			}).catch((error) => {
				showError(t('infomate', 'Error saving selected note'))
				console.error(error)
			})
		},
		onExportNote(noteId) {
			const url = generateOcsUrl('apps/infomate/api/v1/notes/{noteId}/export', { noteId })
			axios.get(url).then(response => {
				showSuccess(t('infomate', 'Note exported in {path}', { path: response.data.ocs.data }))
			}).catch((error) => {
				showError(t('infomate', 'Error deleting note'))
				console.error(error)
			})
